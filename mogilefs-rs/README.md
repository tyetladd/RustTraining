...
```toml
db_path = "mogilefs.db"          # SQLite file backing all metadata
...
MD5/SHA1 `checksum`/`checksumverify`), `get_paths`, `file_info`, `list_keys`, `list_fids`, `delete`,
`rename`, `sleep`, `noop`.
...
Background workers mirror the reference tracker's Monitor/Replicate/Delete workers: periodic disk-usage
stats per device, async replication of newly-closed files up to each class's `mindevcount` (spread across
distinct hosts, weighted by free space — approximating `ReplicationPolicy::MultipleHosts`), and async
physical deletion of unlinked/overwritten fids.

### Not implemented (out of scope for this pass)

`fsck_*`, `rebalance_*`, `httpcopy`, `edit_file` (all advanced maintenance/admin commands beyond the
core client-facing file-storage protocol), the mgmt-port checksum side-channel (checksum verification here
is done via direct local filesystem access instead, since tracker and storage share a process), and
MySQL/PostgreSQL backends (SQLite only). None of these affect wire compatibility for a standard
`MogileFS::Client` doing `new_file`/`read_file`/`delete`/`rename`/`list_keys`-style operations.
...
`tests/protocol.rs` drives the server through the raw tracker line protocol and raw HTTP, exercising the
full `create_open` → `PUT` → `create_close` → `get_paths` → `GET` lifecycle, checksum mismatch handling,
and background replication to `mindevcount`, without any client library — the same bytes a real Perl
client would send.
